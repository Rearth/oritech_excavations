...
public class AdvancementAnnouncer {
    ...
    public static void announce(Announcement announcement) {
        ...
        var soundCandidate = Registries.SOUND_EVENT.getOrEmpty(
            Identifier.of(announcement.sound())
        );
        if (soundCandidate.isEmpty()) {
            Excavation.LOGGER.error(
                "Invalid sound event: {}",
                announcement.sound()
            );
            return;
        ...
        if (currentPlayingSound != null) {
            MinecraftClient.getInstance()
                .getSoundManager()
                .stop(currentPlayingSound);
        }

        var soundEvent = PositionedSoundInstance.master(
            soundCandidate.get(),
            1f,
            1f
        );
        ...
        subtitle = Components.label(
            Text.literal(announcement.subtitles().getFirst().text())
        );
        ...
        var subContainer = Containers.horizontalFlow(
            Sizing.content(1),
            Sizing.content(1)
        );
        subContainer.surface(Surface.flat(color.argb()));

        var lastSubContainer = Containers.horizontalFlow(
            Sizing.content(1),
            Sizing.content(1)
        );
        ...
        var announcerIcon = Components.sprite(
            new SpriteIdentifier(
                SpriteAtlasTexture.BLOCK_ATLAS_TEXTURE,
                Excavation.id("block/announcer_dummy_block")
            )
        );
    ...
    public static void init() {
        mainContainer = Containers.verticalFlow(
            Sizing.content(),
            Sizing.content()
        );
        mainContainer.horizontalAlignment(HorizontalAlignment.RIGHT);

        Hud.add(Excavation.id("hud_progress"), () ->
            mainContainer.positioning(Positioning.relative(100, 35))
        );
    }
    ...
    public static void tick() {
        if (currentAnnouncement != null) {
            ...
            List<Announcement.Subtitle> subtitles =
                currentAnnouncement.subtitles();
            for (int i = 0; i < subtitles.size(); i++) {
                var subtitle = subtitles.get(i);
                if (currentOffset < subtitle.endAt()) {
                    activeSub = subtitle.text();
                    if (i > 0) lastSub = subtitles.get(i - 1).text();
                    break;
                }
            }
            ...
            if (
                currentOffset >=
                currentAnnouncement.subtitles().getLast().endAt()
            ) {
                // end announcement
                mainContainer.clearChildren();
